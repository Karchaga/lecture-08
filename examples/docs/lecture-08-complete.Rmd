...
```{r list-col-geometry}
mutate(lakes, county = str_split(string = county ,
                                 patter = "/"))
```
...
lakes %>%
  mutate(county = str_split(string = county,
                            patter = "/")) %>%
  unnest()
...
###-this will get rid of the geometry in the data for this specific variable-###
...
###-for this case, the counties variable will go on the left because is the variable that contains the geometric data-###
```
...
###-the dsn at the end deletes current version of data that exist before storing new data-###
```
